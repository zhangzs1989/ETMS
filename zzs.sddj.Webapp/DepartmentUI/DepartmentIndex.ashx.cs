...
{
    /// <summary>
    /// DepartmentIndex 的摘要说明
    /// </summary>
    public class DepartmentIndex : IHttpHandler
    {

        public void ProcessRequest(HttpContext context)
        {
            context.Response.ContentType = "text/html";
            string filepath = context.Request.MapPath("DepartmentIndex.html");
            string filecontent = File.ReadAllText(filepath);
            HttpBrowserCapabilities bc = HttpContext.Current.Request.Browser;
            string userloginame = HttpContext.Current.Request.Cookies["userloginame"].Value;
            filecontent = filecontent.Replace("$browertype", bc.Browser).Replace("$browerversion", bc.Version).Replace("$UserDomainName", GetOSVersion()).Replace("$clientip", GetIPAddress).Replace("$loginlastertime", DateTime.Now.ToLocalTime().ToString()).Replace("$department", userloginame);
